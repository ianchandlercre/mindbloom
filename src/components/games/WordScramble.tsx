...
export default function WordScramble({ difficulty, interests, onAnswer, onComplete, currentRound, totalRounds, score, isComplete, initialRounds }: Props) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Hint */}
      {showHint && (
        <div className="text-center mb-4 p-3 bg-amber/10 rounded-warm animate-fade-in flex items-center justify-center gap-2">
        ...
        <div className="flex gap-3 mt-4">
          {!hintUsed && (
            <button
              ...
              disabled={!!feedback}
            >
              <Lightbulb size={16} className="inline mr-1" /> Show Hint
            </button>
          )}
          <button
